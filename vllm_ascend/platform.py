    ...
    def get_attn_backend_cls(cls, selected_backend, attn_selector_config, num_heads: int | None = None):
        ...
        if envs.VLLM_ASCEND_ENABLE_FLASH_MLA:
            if use_sparse or use_compress or attn_selector_config.use_pcp:
                raise ValueError(
                    "FlashMLA requires dense, uncompressed attention with PCP=DCP=1."
                )
            if use_mla:
    ...
    def check_and_update_config(cls, vllm_config: VllmConfig) -> None:
        ...
        if envs.VLLM_ASCEND_ENABLE_FLASH_MLA:
            ...
            if not is_950() or not vllm_config.model_config.use_mla:
                raise ValueError("FlashMLA requires an A5 dense MLA target.")
            if parallel.decode_context_parallel_size != 1 or parallel.prefill_context_parallel_size != 1:
                raise ValueError("FlashMLA requires PCP=DCP=1.")
            if cache.cache_dtype not in ("auto", "bfloat16", "float16"):
                raise ValueError("FlashMLA requires an unquantized KV cache.")
            if vllm_config.kv_transfer_config is not None:
                raise ValueError("FlashMLA does not support KV transfer.")
